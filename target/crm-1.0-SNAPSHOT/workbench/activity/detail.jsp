		...
		$("#saveRemarkBtn").click(function () {
			...
			$.ajax({
				...
				data : {
					...
					"activityId" : "${a.id}"  //忘记加双引号***不然ajax运行不了，导致控制器都进不去
				},
				type : "post",
				dataType : "json",
	...
	function showRemarkList() {
		...
		$.ajax({
			...
			success : function (data) {
				...
				动态生成的，获取需要在字符串中获取，加转义，不然不行（\''+n.id+'\'）
				*/
	...
	function deleteRemark(id) {
		...
		$.ajax({
			...
			success : function (data) {
				if (data.success){
					//删除成功
					//这种做法不行，记录使用的是before方法，每一次删除之后，都会保留原有的数据
					//showRemarkList();
