...
<script type="text/javascript">
	$(function(){
		...
		//为 提交市场活动按钮绑定事件，填充市场活动源(填写两项信息、名字+id)
		...
		$("#convertBtn").click(function () {
			/*
				提交请求到后台，执行线索转换的操作，应该发出传统请求I请求结束后，最终响应回线索列表页
				根据"为客户创建交易"的复选框有没有挑√，来判断是否需要创建交易
			 */
			if ($("#isCreateTransaction").prop("checked")){
				//alert("需要创建交易");
				//如果需要创建交易，除了要为后台传递clueId之外，还得为后台传递交易表单中的信息
				//window.location.href = "workbench/clue/convert.do?clueId=${param.id}&money=xxx&expectedDate=xxx&name=xxx&stage=xx&activityId=xxx";
...
<body>
	...
	<div id="title" class="page-header" style="position: relative; left: 20px;">
		<%--
			el表达式为我们提供了N多个隐含对象只有xxxScope系列的隐含对象可以省略掉
